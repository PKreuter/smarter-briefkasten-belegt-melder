...
class PKCS7:
    ...
    def pad(plaintext: bytearray, block_size: int) -> None:
        ...
        if DEBUG:
            ...
            print('Padding length/value {}'.format(block_size - (len(plaintext) % block_size)))
        if block_size <= 0:
            raise ValueError('block size must be greater than 0')
        padval = block_size - (len(plaintext) % block_size)

        # padding starts with leading 00
        padzero = 0
        plaintext.extend(bytes(padzero for _ in range(1)))
        # rest of padding    
        plaintext.extend(
            bytes(padval for _ in range(padval - 1))
        )
        if DEBUG:
            print('Padded plaintext: {}'.format(plaintext))
